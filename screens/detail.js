...
export default function Detail({ route }){
    ...
    const data = [
        ...
        },
        {
            ...
            "src":require("../assets/porest.png")
        },
    ]
    const postData = async (Id)=>{
        console.log(userId, Id[0]);
        try{
            const res = await fetch(`http://${IP}/solution/save`, {
                method:'POST',
                headers:{
    ...
    return(
        <View style={styles.container}>
            ...
            {data.map((item, index)=>{
                return id.includes(item.solution_id) ?(
                    <View key={index} style={styles.section}>
                    
                    <View style={styles.titleBox}>
                    <Text style={styles.title}>{item.title}</Text>
                    <TouchableOpacity style={styles.btn} onPress={()=>postData(id)}>
                        <Check width={20}/>
                        <Text style={styles.btnText}>저장하기</Text>
                    </TouchableOpacity>
                    
                    </View>
                        <Image source={item.src}  style={[{marginBottom:20}]}/>
            ...
            <Footer people={false} home={true} storage={false} />
        </View> 
    )
}
